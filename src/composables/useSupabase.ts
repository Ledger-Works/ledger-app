...
import type { Expense, NewExpense, ExpenseWithDetails, Group, UserGroupRole, ExpenseSplit, ExpenseResponse, GroupResponse } from '@/types'
...
export const useSupabase = () => {
	...
	const init = () => {
		...
		supabase.value = createClient('https://hgugshdprcthbctifilw.supabase.co', apiKey);
	}

	const getExpenses = async (): Promise<ExpenseWithDetails[] | undefined> => {
		try {
        ...
        `)
			...
			const expensesWithSplits = await Promise.all(data.map(async (expense: ExpenseResponse) => {
			...
			errors.showError('Error fetching expenses', errors.getErrorMessage(error))
		}
	}
	...
	const getUserGroupRoles = async (userId: string): Promise<UserGroupRole[] | undefined> => {
		try {
			const { data, error } = await supabase.value
			...
			errors.showError('Error fetching user group roles', errors.getErrorMessage(error))
		}
	}
	...
	const addExpenseSplit = async (split: ExpenseSplit): Promise<void> => {
		try {
			...
			errors.showError('Error adding expense split', errors.getErrorMessage(error))
		}
	}

	const createGroup = async (group: Omit<Group, 'id'>, creatorUserId: string): Promise<Group | undefined> => {
		try {
	...
	return { init, getExpenses, addExpense, getGroups, getUserGroupRoles, addExpenseSplit, createGroup }
}